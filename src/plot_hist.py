...
def get_list(matrix_path):
    ...
    for i in range(1000):
        ...
        with open(shard_folder+"/neighbor_list.txt") as f:
            lines = f.readlines()
        for line in lines:
...
def plot_hist(val_list, cut_off):
    ...
    count_100_inf = sum(v > 100 for v in val_list)
    ...
    # Save counts to CSV
    df = pd.DataFrame({
        "Bin": ["{0}", "(0,10]", "(10,100]", "(100,∞)"],
        "Count": [count_0, count_0_10, count_10_100, count_100_inf]
    })

    csv_fn = f'neighbor_hist_filter_{cut_off}_counts.csv'
